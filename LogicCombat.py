...
def _can_combat_generic(world: 'MetroidPrimeWorld', state: CollectionState, normal_tanks: int, minimal_tanks: int, requires_charge_beam: bool = True) -> bool:
    ...
    if difficulty == CombatLogicDifficulty.NO_LOGIC.value:
        ...
        return has_energy_tanks(world, state, normal_tanks) and (can_charge_beam(world, state) or not requires_charge_beam)
    elif difficulty == CombatLogicDifficulty.MINIMAL.value:
        return has_energy_tanks(world, state, minimal_tanks) and (can_charge_beam(world, state) or not requires_charge_beam)
    return True
...
def can_combat_thardus(world: 'MetroidPrimeWorld', state: CollectionState) -> bool:
    ...
    if world.options.starting_room_name.value in [RoomName.Quarantine_Monitor.value, RoomName.Save_Station_B.value]:
        return can_plasma_beam(world, state) or can_power_beam(world, state) or can_wave_beam(world, state)
    difficulty = world.options.combat_logic_difficulty.value
    if difficulty == CombatLogicDifficulty.NO_LOGIC.value:
        return True
    elif difficulty == CombatLogicDifficulty.NORMAL.value:
        return has_energy_tanks(world, state, 3) and (can_charge_beam(world, state) and (can_plasma_beam(world, state) or can_power_beam(world, state)))
    elif difficulty == CombatLogicDifficulty.MINIMAL.value:
        return can_plasma_beam(world, state) or can_power_beam(world, state) or can_wave_beam(world, state)
    return True
...
def can_combat_ghosts(world: 'MetroidPrimeWorld', state: CollectionState) -> bool:
    ...
    if difficulty == CombatLogicDifficulty.NO_LOGIC.value:
        ...
        return can_charge_beam(world, state, SuitUpgrade.Power_Beam) and can_power_beam(world, state) and can_xray(world, state, True)
    elif difficulty == CombatLogicDifficulty.MINIMAL.value:
        return can_power_beam(world, state)
    return True


def can_combat_beam_pirates(world: 'MetroidPrimeWorld', state: CollectionState, beam_type: SuitUpgrade) -> bool:
