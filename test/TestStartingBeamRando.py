...
class TestStartingBeamRandomizationDisabled(MetroidPrimeTestBase):
    ...
    def test_power_beam_is_the_starting_weapon_when_not_randomized(self):
        ...
        world.create_items()
        self.assertGreater(len(world.multiworld.precollected_items[world.player]), 0)
        self.assertIn(SuitUpgrade.Power_Beam.value, [item.name for item in world.multiworld.precollected_items[world.player]])

    def test_hive_mecha_is_not_disabled_if_starting_at_landing_site_and_power_beam_is_starting_weapon(self):
        self.world_setup()
        world: 'MetroidPrimeWorld' = self.world
        distribute_items_restrictive(self.multiworld)
        config = make_config(world)
