...
def get_world_blast_shield_mapping(world: 'MetroidPrimeWorld') -> WorldBlastShieldMapping:
    ...
    if world.options.locked_door_count > 0:
        ...
        areas_with_locks = world.random.sample([area for area in list(MetroidPrimeArea) if area != MetroidPrimeArea.Magmoor_Caverns], world.options.locked_door_count.value)
...
def _generate_blast_shield_mapping_for_area(world: 'MetroidPrimeWorld', area: MetroidPrimeArea, include_locked_door: bool) -> BlastShieldMapping:
    ...
    if cast(str, world.options.blast_shield_randomization.value) == BlastShieldRandomization.option_mix_it_up:
        ...
        for i in range(num_shields_to_add):
            ...
            door_data = get_door_data_by_room_names(source_room, region.doors[source_room], area, world)
    ...
    elif cast(str, world.options.blast_shield_randomization.value) == BlastShieldRandomization.option_replace_existing:
...
def _get_available_blast_shields(world: 'MetroidPrimeWorld', force_exclude_combo_doors: bool = False) -> List[BlastShieldType]:
    available_shields = [shield for shield in ALL_SHIELDS.copy() if shield not in [BlastShieldType.Disabled, BlastShieldType.No_Blast_Shield]]
    if cast(str, world.options.blast_shield_randomization.value) == BlastShieldRandomization.option_replace_existing:
