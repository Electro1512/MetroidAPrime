    ...
    def __init__(self):
        ...
        self.rooms = {
            ...
            RoomName.Fiery_Shores: RoomData(doors={
                0: DoorData(RoomName.Shore_Tunnel, rule_func=lambda world, state: can_heat(world, state) and (can_bomb(world, state) or can_grapple(world, state) or has_energy_tanks(world, state, 4))),
                1: DoorData(RoomName.Transport_Tunnel_B, destination_area=MetroidPrimeArea.Magmoor_Caverns, rule_func=lambda world, state: can_heat(world, state) and (can_bomb(world, state) or can_grapple(world, state))),
                ...
                PickupData('Magmoor Caverns: Fiery Shores - Warrior Shrine Tunnel', rule_func=lambda world, state: can_power_bomb(world, state) and can_bomb(world, state) and (can_warp_to_start(world, state) if world.options.starting_room_name == RoomName.Warrior_Shrine.value else state.can_reach(RoomName.Warrior_Shrine.value, None, world.player)))  # Not an item in this room but can only be accessed from here, if starting in warrior shrine need to be able to warp back
            ]),
            RoomName.Geothermal_Core: RoomData(doors={
                ...
                1: DoorData(RoomName.Plasma_Processing, defaultLock=DoorLockType.Ice,
                            rule_func=lambda world, state: can_bomb(world, state) and can_boost(world, state) and can_grapple(world, state) and can_spider(world, state) and can_space_jump(world, state),
            ...
            RoomName.Lava_Lake: RoomData(
                doors={
                    0: DoorData(RoomName.Lake_Tunnel, destination_area=MetroidPrimeArea.Magmoor_Caverns, rule_func=lambda world, state: can_heat(world, state) and (can_bomb(world, state) or can_power_bomb(world, state)), tricks=[Tricks.lava_lake_item_suitless]),
                    1: DoorData(RoomName.Pit_Tunnel, rule_func=lambda world, state: can_heat(world, state) and (can_bomb(world, state) or can_power_bomb(world, state)), tricks=[Tricks.lava_lake_item_suitless]),
                },
                pickups=[PickupData('Magmoor Caverns: Lava Lake', rule_func=lambda world, state: can_missile(world, state) and can_space_jump(world, state) and state.can_reach("Magmoor Caverns: " + RoomName.Lake_Tunnel.value, None, world.player), tricks=[Tricks.lava_lake_item_missiles_only, Tricks.lava_lake_item_suitless]), ]),
            RoomName.Magmoor_Workstation: RoomData(
                ...
                pickups=[PickupData('Magmoor Caverns: Magmoor Workstation', rule_func=lambda world, state: can_thermal(world, state) and can_wave_beam(world, state) and can_scan(world, state) and can_morph_ball(world, state),
            ...
            RoomName.Monitor_Station: RoomData(doors={
                ...
                2: DoorData(RoomName.Warrior_Shrine, rule_func=lambda world, state: can_heat(world, state) and can_space_jump(world, state) and can_boost(world, state), tricks=[Tricks.warrior_shrine_no_boost, Tricks.warrior_shrine_scan_only, Tricks.warrior_shrine_no_items]),
            ...
            RoomName.Pit_Tunnel: RoomData(doors={
                0: DoorData(RoomName.Triclops_Pit, rule_func=lambda world, state: can_heat(world, state) and (can_morph_ball(world, state) or can_space_jump(world, state))),
                1: DoorData(RoomName.Lava_Lake, rule_func=lambda world, state: can_heat(world, state) and (can_morph_ball(world, state) or can_space_jump(world, state))),
            }),
            ...
            }, pickups=[PickupData('Magmoor Caverns: Shore Tunnel', rule_func=lambda world, state: can_power_bomb(world, state) and can_space_jump(world, state), tricks=[Tricks.shore_tunnel_escape_no_sj]), ],),
            ...
            RoomName.Transport_Tunnel_A: RoomData(doors={
                0: DoorData(RoomName.Transport_to_Phendrana_Drifts_North, rule_func=lambda world, state: can_heat(world, state) and can_bomb(world, state)),
                1: DoorData(RoomName.Monitor_Station, rule_func=lambda world, state: can_heat(world, state) and can_bomb(world, state)),
            }, pickups=[PickupData('Magmoor Caverns: Transport Tunnel A', rule_func=lambda world, state: can_heat(world, state) and can_bomb(world, state))], include_area_in_name=True),
            RoomName.Transport_Tunnel_B: RoomData(doors={
                0: DoorData(RoomName.Transport_to_Tallon_Overworld_West, rule_func=lambda world, state: can_heat(world, state) and can_morph_ball(world, state), tricks=[Tricks.transport_tunnel_b_damage_boost]),
                1: DoorData(RoomName.Fiery_Shores, rule_func=lambda world, state: can_heat(world, state) and can_morph_ball(world, state), tricks=[Tricks.transport_tunnel_b_damage_boost]),
            }, include_area_in_name=True),
            ...
            RoomName.Triclops_Pit: RoomData(
                doors={
                    0: DoorData(RoomName.Monitor_Tunnel, rule_func=lambda world, state: can_heat(world, state)),
                    1: DoorData(RoomName.Storage_Cavern, rule_func=lambda world, state: can_heat(world, state) and can_morph_ball(world, state)),
                    2: DoorData(RoomName.Pit_Tunnel, rule_func=lambda world, state: can_heat(world, state)),
                },
                pickups=[PickupData('Magmoor Caverns: Triclops Pit', rule_func=lambda world, state: can_xray(world, state) and can_space_jump(world, state) and can_missile(world, state),
                                    tricks=[
            ...
            RoomName.Twin_Fires_Tunnel: RoomData(doors={
                0: DoorData(RoomName.Twin_Fires, rule_func=lambda world, state: can_heat(world, state) and can_spider(world, state), tricks=[Tricks.twin_fires_tunnel_no_spider, Tricks.cross_twin_fires_suitless]),
                1: DoorData(RoomName.Transport_to_Tallon_Overworld_West, rule_func=lambda world, state: can_heat(world, state) and (can_spider(world, state) or (can_move_underwater(world, state) or can_space_jump(world, state)))),  # Can't jump out of the lava without gravity or sj
            }),
            RoomName.Twin_Fires: RoomData(doors={
                0: DoorData(RoomName.North_Core_Tunnel, defaultLock=DoorLockType.Wave, rule_func=lambda world, state: (can_space_jump(world, state) and has_energy_tanks(world, state, 2)) or can_grapple(world, state)),
                1: DoorData(RoomName.Twin_Fires_Tunnel, rule_func=lambda world, state: (can_space_jump(world, state) and has_energy_tanks(world, state, 2)) or can_grapple(world, state)),
            }),
            RoomName.Warrior_Shrine: RoomData(doors={
                0: DoorData(RoomName.Monitor_Station, rule_func=lambda world, state: can_heat(world, state)),
                1: DoorData(RoomName.Fiery_Shores, defaultLock=DoorLockType.None_, rule_func=lambda world, state: can_heat(world, state) and can_power_bomb(world, state) and can_bomb(world, state), exclude_from_rando=True),
